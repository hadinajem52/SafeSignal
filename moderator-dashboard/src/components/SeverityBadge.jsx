...
function SeverityBadge({ severity, variant = SEVERITY_VARIANTS.REPORTS, display = 'pill' }) {
  ...
  if (display === 'initial') {
    ...
    return (
      ...
      >
        <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.05em', color, textTransform: 'uppercase', lineHeight: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 2 }}>
          {[0.25, 0.55, 1].map((opacity, i) => (
            <div
              key={i}
              style={{ width: 5, height: 5, borderRadius: '50%', background: color, opacity }}
            />
          ))}
        </div>
      </div>
