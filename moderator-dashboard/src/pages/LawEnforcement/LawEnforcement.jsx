...
const LEI_QUEUE_WIDTH = { min: 320, max: 760, default: 420 };
const LEI_SPLITTER_WIDTH = 8;
const LEI_MIN_DETAIL_WIDTH = 520;
    ...
    >
      <div className="lei-ops-map-header">
        ...
        <div className="lei-ops-map-sub">
          Click a marker to select an incident in the Queue.
        </div>
      </div>
...
function LawEnforcement() {
  ...
  const clampQueueWidth = useCallback((nextWidth, containerWidth) => {
    const availableWidth = containerWidth - LEI_SPLITTER_WIDTH;
    if (availableWidth <= 0) return nextWidth;
    const maxFromLayout = Math.max(
      LEI_QUEUE_WIDTH.min,
      availableWidth - LEI_MIN_DETAIL_WIDTH,
    );
    return Math.min(
      Math.max(nextWidth, LEI_QUEUE_WIDTH.min),
      Math.min(LEI_QUEUE_WIDTH.max, maxFromLayout),
    );
  }, []);

  const handleQueueSplitterPointerDown = (event) => {
  ...
  const handleQueueSplitterKeyDown = (event) => {
  ...
  const notifyCriticalAlert = async (payload) => {
    if (typeof Notification === "undefined" || payload?.severity !== "critical")
      return;
    const show = () => {
