...
import { Radio, CheckCircle2, AlertTriangle, MapPin, ChevronRight } from 'lucide-react'
    ...
    <>
        ...
        <div className="flex items-start justify-between mb-1">
          <div>
            <div className="flex items-center gap-1.5 mb-1">
              ...
              <span className="text-[11px] text-muted font-semibold uppercase tracking-widest">Active Deployments</span>
            </div>
      ...
      <SectionCard title="Cases by Stage">
        {stageCounts.map(({ status, count }, i) => (
          <div key={status} className={`flex items-center gap-3 py-2.5 ${i < stageCounts.length - 1 ? 'border-b border-border' : ''}`}>
            ...
            <span className="flex-1 text-xs text-text capitalize min-w-0">{status.replace(/_/g, ' ')}</span>
      ...
      <SectionCard title="Recently Closed">
        {recentlyClosed.length === 0 ? (
          ...
          <div key={inc.incident_id} className={`flex items-center gap-2 py-2.5 ${i < recentlyClosed.length - 1 ? 'border-b border-border' : ''}`}>
            <CheckCircle2 size={11} className="text-success flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-xs text-text truncate">{inc.title || `Case #${inc.incident_id}`}</p>
              <p className="text-[10px] text-muted">{getTimeAgo(inc.updated_at || inc.created_at)}</p>
            </div>
            <ChevronRight size={12} className="text-muted flex-shrink-0" />
          </div>
        ))}
  ...
  return (
    <>
      <div className="grid grid-cols-2 gap-4">
        <BigStatTile label="Active Cases" value={activeCases.length} icon={Radio} iconColor="text-warning" loading={loading} />
        <BigStatTile label="Dispatched" value={dispatched.length} icon={MapPin} iconColor="text-primary" loading={loading} />
      </div>
      ...
      <SectionCard title="Unactioned Alerts">
        {loading ? (
          ...
          <div className="flex items-center gap-2 py-3">
            ...
            <p className="text-xs text-success font-medium">All verified cases actioned within 30 min</p>
          </div>
        ) : unactioned.map((inc, i) => (
          <div key={inc.incident_id} className={`flex items-center gap-2.5 py-2.5 ${i < unactioned.length - 1 ? 'border-b border-border' : ''}`}>
      ...
      <SectionCard title="Incoming Cases">
        {loading ? (
          ...
          <p className="text-xs text-muted text-center py-3">No new incoming cases</p>
        ) : incoming.map((inc, i) => (
          <IncidentRow
            key={inc.incident_id} inc={inc} idx={i} total={incoming.length}
...
export function LeiRight({ leiIncidents, loading }) {
  ...
  const nearClosure = leiIncidents.filter((i) => i.status === 'investigating').slice(0, 5)
  const allStages = ['verified', 'dispatched', 'on_scene', 'investigating', 'police_closed'].map((st) => ({
    status: st,
    count: leiIncidents.filter((i) => i.status === st).length,
  }))
    ...
    <>
      <SectionCard title="Severity Breakdown">
        <div className="grid grid-cols-2 gap-2">
          {sev.map(({ label, count, color }) => (
            <div key={label} className="bg-surface rounded-lg px-2 py-2.5 text-center">
              <p className={`text-lg font-bold font-display leading-none ${color}`}>
                {loading ? '—' : count}
              </p>
      ...
      <SectionCard title="Near Closure">
        {loading ? (
          ...
          <p className="text-xs text-muted text-center py-3">No cases in investigating stage</p>
        ) : nearClosure.map((inc, i) => (
          <div key={inc.incident_id} className={`flex items-center gap-2 py-2.5 ${i < nearClosure.length - 1 ? 'border-b border-border' : ''}`}>
      ...
      <SectionCard title="All Stages">
        {allStages.map((item, i) => (
          <div key={item.status} className={`flex items-center gap-2 py-2 ${i < allStages.length - 1 ? 'border-b border-border' : ''}`}>
            <span className="flex-1 text-xs text-text capitalize">{item.status.replace(/_/g, ' ')}</span>
            <span className={`text-xs font-bold ${leiStatusColor(item.status)}`}>
              {loading ? '—' : item.count}
            </span>
            <ChevronRight size={12} className="text-muted flex-shrink-0" />
          </div>
