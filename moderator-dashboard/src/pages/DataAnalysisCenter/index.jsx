        ...
        <div className="dac-scroll">
          ...
          <div className="dac-section-row">
            ...
            <div className="dac-section-meta">
              Last {period} · {incidents.length} reports
            </div>
          </div>
                  ...
                  <div
                    ...
                    onMouseEnter={(e) =>
                      showTip(e, `${p.label} Percentile`, [
                        ...
                        {
                          label: "Meaning",
                          value: `${p.fill}% of cases under this`,
                        },
                      ])
                    }
                    onMouseMove={moveTip}
          ...
          <div className="dac-card" style={{ marginBottom: 20 }}>
            <div className="dac-card-header">
              ...
              >
                ↑ Peak: {DAYS_LABELS[peakDow]}{" "}
                {String(peakHour).padStart(2, "0")}:00 ({peakCount} incidents)
              </div>
            </div>
          ...
          <div className="dac-grid-3col">
            <div className="dac-card">
              ...
              <div className="dac-funnel-list">
                {funnelData.map((f, i) => {
                  ...
                  const drop =
                    prev && prev > 0
                      ? (((prev - f.count) / prev) * 100).toFixed(0)
                      : null;
                  const funnelPct =
                    funnelData[0].count > 0
                      ? ((f.count / funnelData[0].count) * 100).toFixed(0)
                      : 0;
                  return (
                    <div
                      key={f.label}
                    ...
                    >
                      ...
                      <div className="dac-funnel-drop">
                        {drop ? (
                          <span style={{ color: "var(--dac-red)" }}>
                            −{drop}%
                          </span>
                        ) : (
                          <span style={{ color: "var(--dac-muted)" }}>—</span>
                        )}
                      </div>
                    </div>
                  );
              ...
              {(() => {
                ...
                const dropPct =
                  verified > 0
                    ? (((verified - dispatched) / verified) * 100).toFixed(0)
                    : 0;
                return (
                  verified > 0 && (
                    <div className="dac-alert-footer">
                      ...
                      >
                        {dropPct}% drop from Verified → Dispatched — investigate
                        backlog
                      </span>
                    </div>
                  )
            ...
            <div className="dac-card">
                  ...
                  {[0, 1, 2, 3].map((wk) => (
                    <div key={wk} className="dac-cat-group">
                      {activeCats.map(([cat]) => {
                        ...
                        const catColor =
                          CAT_DISPLAY[cat]?.color || "var(--dac-muted)";
                        return (
                          <div
                            key={cat}
                            ...
                            onMouseEnter={(e) =>
                              showTip(
                                e,
                                `${CAT_DISPLAY[cat]?.label || cat} — ${wkLabel}`,
                                [
                                  {
                                    dot: catColor,
                                    label: "Incidents",
              ...
              <div className="dac-cat-legend">
                {activeCats.length === 0 ? (
                  <div style={{ fontSize: 10, color: "var(--dac-muted)" }}>
                    No category data in range
                  </div>
                ) : (
                  activeCats.map(([cat]) => (
            ...
            <div className="dac-card">
              ...
              <div className="dac-sla-breakdown">
                {[
                  ...
                  {
                    label: "Cases Sampled",
                    value: kpis.responseTimes.length,
                    color: "var(--dac-blue)",
                  },
                ].map((s) => (
          ...
          <div className="dac-grid-2">
            <div className="dac-card">
              ...
              <div className="dac-info-footer">
                Incident count over last {period} · Red = recurring hotspot
              </div>
            </div>
            ...
            <div className="dac-card">
                ...
                reporterStats.map((r, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className="dac-quality-bar"
                        style={{ marginLeft: "auto" }}
                      >
                        <div className="dac-quality-track">
                          <div
                            className="dac-quality-fill"
                            style={{ width: `${r.pct}%`, background: r.color }}
                          />
                        </div>
                      </div>
                      <div
                        className="dac-reporter-stat"
                        style={{ marginLeft: 10 }}
                      >
                        <div
                          className="dac-reporter-pct"
                          style={{ color: r.color }}
                        >
                          {r.pct}%
                        </div>
              ...
              {reporterStats.some((r) => r.pct === 0) && (
                <div className="dac-alert-footer">
                  ...
                  >
                    {reporterStats.filter((r) => r.pct === 0).length}{" "}
                    reporter(s) with 0% accuracy — flag for manual review.
                  </span>
                </div>
              )}
            </div>
