export default function FunnelCard({ funnelData, showTip, moveTip, hideTip }) {
  const verifiedCount = funnelData[1]?.count || 0;
  const dispatchCount = funnelData[2]?.count || 0;
  const verifyDropPct =
    verifiedCount > 0
      ? (((verifiedCount - dispatchCount) / verifiedCount) * 100).toFixed(0)
      : 0;
    ...
    <div className="dac-card">
          ...
          return (
            ...
            >
              ...
              <div className="dac-funnel-drop">
                {drop ? (
                  <span style={{ color: "var(--dac-red)" }}>−{drop}%</span>
                ) : (
      ...
      {verifiedCount > 0 && (
        <div className="dac-alert-footer">
          ...
          <span
            style={{ fontSize: 10, fontWeight: 600, color: "var(--dac-amber)" }}
          >
            {verifyDropPct}% drop from Verified → Dispatched — investigate
            backlog
          </span>
        </div>
      )}
