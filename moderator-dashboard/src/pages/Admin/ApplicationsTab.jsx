...
function ApplicationsTab({
  ...
  const selectedRisk = selected
    ? assessRisk(selected.email, selected.role)
    : null;
    ...
    <div className="flex flex-1 overflow-hidden min-h-0 border border-border">
            ...
            filtered.map((app) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-[10px] text-muted mb-1.5 truncate">
                    {app.email}
                  </p>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 border border-border">
                <MetaCell
                  label="Applied"
                  value={new Date(selected.appliedAt).toLocaleString()}
                />
                <MetaCell
                  label="Requested Role"
                  value={
                    selected.role === "law_enforcement"
                      ? "Law Enforcement"
                      : "Moderator"
                  }
      ...
      <ConfirmDialog
        ...
        message={`Grant ${confirmApprove?.username ?? "this user"} access as ${
          confirmApprove?.role === "law_enforcement"
            ? "Law Enforcement"
            : "Moderator"
...
function RiskBlock({ level, note }) {
  ...
  return (
    <div className={`flex items-start gap-3 p-3.5 border ${s.block}`}>
      <Icon
        size={15}
        className={`flex-shrink-0 mt-0.5 ${s.iconColor}`}
        strokeWidth={2}
      />
      <div>
        <p
          className={`text-[10px] font-bold tracking-widest uppercase mb-1 ${s.iconColor}`}
        >
          {s.label}
        </p>
